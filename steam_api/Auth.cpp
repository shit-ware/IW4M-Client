...
void Auth_Error(const char* message)
{
	...
	MessageBoxW(NULL, buf, L"Authentication Error", MB_OK | MB_ICONSTOP);
}
...
bool Auth_PerformSessionLogin(const char* username, const char* password)
{
	...
	if (curl)
	{
		...
		#endif
		//_snprintf(url, sizeof(url), "http://%s/remauth.php", "www.repziw4.de");
		...
		//curl_easy_setopt(curl, CURLOPT_USERAGENT, "aIW3");
		...
		{
			const int result = MessageBox(NULL, "Could not reach the RepZ servers. Would you like to play offline perhaps?", "Damn, so signal ;(", MB_YESNOCANCEL);
			...
			case IDYES:
				...
				break;
			default:
				Auth_Error(va("Could not reach the %s server. Error code from CURL: %x.", "RepZIW4M", code));
			}
		}
...
void Auth_OpenDLCStore()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://repziw4.net/content/dlc.txt");
