...
bool FS_GetFileForDownload(const char* path, char* buffer, size_t bufferSize)
{
	...
	// check if we are doing a fastfile
	const char* ff = va("%s\\%s", (*fs_basepath)->current.string, path);
	// dont let people download shit not in the mods folder
	if (!strncmp((*fs_game)->current.string, path, strlen((*fs_game)->current.string)) 
		&& FileExists(ff))  // check to see if it exists in the fs
	{
		strcpy_s(buffer, bufferSize, ff);
		return true;
	}
