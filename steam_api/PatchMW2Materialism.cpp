...
Material* Material_Register(const char* filename)
{
	...
	if (!_stricmp(filename, "hud_colt45"))
	{
		return Material_Obtain(filename, filename);
	}
	...
	if (!_stricmp(filename, "weapon_peacekeeper"))
	{
		return Material_Obtain(filename, filename);
	}

	if (!_stricmp(filename, "weapon_ak74"))
	{
		return Material_Obtain(filename, filename);
	}

	if (!_stricmp(filename, "weapon_w1200"))
	{
		return Material_Obtain(filename, filename);
	}

	if (!_stricmp(filename, "weapon_thompson"))
	{
		return Material_Obtain(filename, filename);
	}
	...
	if (!_stricmp(filename, "weapon_r700"))
	{
		return Material_Obtain(filename, filename);
	}
