...
int* fs_numServerReferencedFFs = (int*)0x63D4FDC;
int* fs_serverReferencedFFs = (int*)0x63D2E58;
char** fs_serverReferencedFFNames = (char**)0x63D2DD8;
...
int FS_CompareWithServerFiles(char* needediwds, int len, bool dlstring)
{
	...
	*needediwds = 0;
	...
	// check about the referenced FF's
	/*
	for(i=0; i<*fs_numServerReferencedFFs; i++)
	{
	// ignore the stock fastfiles
	if(GetZoneLocation(fs_serverReferencedFFNames[i], false)) continue;

	// see if it exists locally
	const char* ff = va("%s\\%s\\%s.ff", (*fs_basepath)->current.string, (*fs_game)->current.string, fs_serverReferencedFFNames[i]);
	// TODO: some sort of checksum here like above?
	if (!FileExists(ff)) {
	// Remote name
	strncat( needediwds, "@", len);
	strncat( needediwds, (*fs_game)->current.string, len);
	strncat( needediwds, "/", len);
	strncat( needediwds, fs_serverReferencedFFNames[i] , len);
	strncat( needediwds, ".ff" , len);
...
void GetFS_Game()
{
	...
	// if we have a mod loaded then we need to vid_restart in order to see the iwd files
	// this might piss some people off even if iwds arent used and they have to vid_restart
	// just to get into a modded server.... well let them turn that off and fuck themselves
	if (cl_modVidRestart->current.boolean)
	{
		char* game = Info_ValueForKey(msg, "fs_game");
		if (strlen(game))
		{
			(*fs_game)->pad2[0] = 1; // force filesystem refresh so we see the new IWD files
			__asm
			{
				mov eax, 4CA1F0h // CL_Vid_Restart_f
					call eax
