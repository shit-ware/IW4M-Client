...
const char* TheatreMenu_GetItemText(int index, int column)
{
	...
	for (int i = 0; i < filenum; i++)
	{
		...
		if (len >= 6) {
			if (!strstr(list[i], "demofix_")) {
				...
				meta = fopen(va(".\\m2demo\\demos\\%s.meta", tmp.demoName), "r");
				if (meta != NULL) {
...
void customRecordFunc()
{
	...
	if (!recording)
	{
		...
		if (Cmd_Argc() == 2) {
			...
			if (!fexist((char*)va(".\m2demo\\demos\\%s.dm_13", curDemoName))) {
			...
			{
				if (fexist((char*)va(".\\m2demo\\demos\\demo%04d.dm_13", i))) {
...
void customStopRecordFunc()
{
	if (recording) {
		...
		metaFile = fopen(va(".\\m2demo\\demos\\%s.meta", curDemoName), "w");
		if (metaFile == NULL) {
		...
		bool copy = CopyFile(va(".\\%s\\demos\\%s.dm_13", fs_game->current.string, curDemoName), va(".\\m2demo\\demos\\%s.dm_13", curDemoName), true);
		if (copy > 0)
			DeleteFile(va(".\\%s\\demos\\%s.dm_13", fs_game->current.string, curDemoName));
		else
			Com_Printf(1, "Failed to copy demo file to m2demo folder... is it readonly?\n");
