...
void PatchMW2()
{
	...
	{ // 1.0.159 (iw4m) version-specific address
		...
		{
			Auth_VerifyIdentity();
			CreateThread(0, 0, getDediVersion, 0, 0, 0);
...
void PatchMW2_159()
{
	...
	*(WORD*)0x6B8B9F = 0x9090;
	...
	//PatchMW2_FrameRate();
	PatchMW2_StaffColour();
	PatchMW2_Packet();
	PatchMW2_IGCAC();
	PatchMW2_MissingModels();
	PatchMW2PhysicPreset();
	PatchMW2_PlayerdataDefinition();

	PatchMW2_TheatreMenu();
...
void CL_OpenStore_f()
{
	ShellExecute(NULL, "open", "http://repziw4.de/viewforum.php?f=27", 0, 0, SW_SHOWNORMAL);
	Auth_OpenDLCStore();
}
...
void PatchMW2_Client()
{
	...
	*(DWORD*)0x799658 = 1600;
	*(DWORD*)0x799918 = (DWORD)malloc(0x684 * 1600);
...
void* MissingModelHook(int type, char* name)
{
	if (!strncmp("void", name, 5))
		return DB_FindXAssetHeader(ASSET_TYPE_XMODEL, "tag_origin");
...
{
#ifdef DEVMODE
	call(0x659457, MissingModelHook, PATCH_CALL);
#endif
}