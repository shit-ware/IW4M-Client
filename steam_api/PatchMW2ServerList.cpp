...
int dediVersion = 0;//Hold the latest dedicated server version. (http://www.repziw4.net/content/dediiw4/minversion.txt)
...
size_t DediVersionReceived(void *ptr, size_t size, size_t nmemb, void *data)
{
	size_t rsize = (size * nmemb);
	char* text = (char*)ptr;
	dediVersion = atoi(text);
	return rsize;
}
...
static void CleanColors(const char* s, char* d, bool patch_ascii = false) //Merged with ASCIIEscape code
{
	while (*s != 0)
	{
		if (*s == '^' && !patch_ascii)
		...
		else if ((*s >= '0' && *s <= '9') || (*s >= 'a' && *s <= 'z') || (*s >= 'A' && *s <= 'Z') || ((*s == '^' || *s == ' ' || *s == '.' || *s == '[' || *s == ']' || *s == '{' || *s == '}' || *s == '=' || *s == '|' || *s == '/' || *s == ':' || *s == '&' || *s == '(' || *s == ')' || *s == '~' || *s == '@') && patch_ascii))
		{
			*d = *s;
			d++;
		}
...
static void LAN_GetServerInfo(int source, int n, char *buf, int buflen) {
	...
	if (server && buf) {
		...
		// Remove non-latin chars form the hostname
		char cleaned[1024];
		CleanColors(server->hostName, cleaned, true);

		// Don't display server if client amount exceeds 18
		if( server->clients > MAX_CLIENTS)
			strncpy(server->version, "\0", 128);

		Info_SetValueForKey(info, "hostname", cleaned);
...
static void UI_StopServerRefresh(void)
{
	...
	if (count - serverStatus.numDisplayServers > 0) {
		Com_Printf(0, "%d servers not listed due to packet loss, pings higher than %d\n",
...
static void UI_BuildServerDisplayList(int force) {
	...
	for (i = 0; i < count; i++)
	{
		...
		/*
		utter bullshit
		if (strcmp(Info_ValueForKey(info, "shortversion"), "3.0-1") != 0)
		continue;
...
DWORD WINAPI getDediVersion(LPVOID) {
	//Get latest dedi version
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://www.repziw4.net/content/dediiw4/minversion.txt");

		curl_easy_setopt(curl, CURLOPT_URL, url);
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, DediVersionReceived);
