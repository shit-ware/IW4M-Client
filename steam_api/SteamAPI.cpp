...
int Auth_GetUserID();
void updateStatus(const char* mode,const char* value);
...
size_t UDataReceived(void *ptr, size_t size, size_t nmemb, void *data)
{
	...
	{
		char* message;
		sprintf(message,"This version (%d) has expired. Delete cachesSxS.xml and run LaunchIW4M.exe to obtain a new version (%d).\n", BUILDNUMBER, version);
		...
		if (result == IDYES)
		{
			...
			ShellExecute(NULL, "open", "LauncherV3.exe", 0, 0, SW_SHOWNORMAL);
			ExitProcess(1);
		}
		else if (result == IDNO)
		{
...
void DisableOldVersions()
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://www.repziw4.net/content/clientrepzstyle/version.txt");
